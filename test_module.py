...
# Helper to recursively compare nested lists/dicts with arrays and floats
def deep_almost_equal(d1, d2, tol=1e-6):
    for key in d1:
        a = d1[key]
        b = d2[key]
        for x, y in zip(a, b):
            if isinstance(x, np.ndarray) and isinstance(y, (list, np.ndarray)):
...
class UnitTests(unittest.TestCase):
    def test_calculate(self):
        ...
        self.assertTrue(deep_almost_equal(actual, expected), "Output didn't match expected")
